...
function is_registered(username) {
	...
	if (username == '') {
		...
		$.post(
			"/Estore_raw/user?method=checkUsernameIsExist", // url
			{ "username" : username }, // data
			function(flag){ // 回调函数
				var checkInfo = "";
				if(!flag){
					// 用户名存在
					checkInfo = "用户名已存在,请重新输入!";
					$("#checkUername").css("color", "red");
				} else {
					// 用户名不存在
					checkInfo = "用户名不存在,可以使用";
					$("#checkUername").css("color", "green");
				}
				$("#checkUername").html(checkInfo);
			},
			"json"
		);
		return true;
...
function check_captcha() {
	...
	if (captcha.length == 0) {
		...
		$.post(
				"/Estore_raw/user?method=imgCodeCheck", // 1.后台处理的uri
				{
					"captcha" : captcha  // 2.代表请求服务器端的数据,传到服务器
				}, 
				function(data) { // 3.回调函数  data? 根据回传信息 ，动态显示提示信息
					...
					if(!data){
						...
						checkInfo = "验证码正确";
						$("#checkImgInfo").css("color", "green");
					}
					$("#checkImgInfo").html(checkInfo);
				},
				"json" // 4.数据类型
			);
		return true;
